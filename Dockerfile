...
# Phoenix
ARG PHOENIX_VERSION=4.9.0
RUN curl -s https://archive.apache.org/dist/phoenix/apache-phoenix-$PHOENIX_VERSION-HBase-$HBASE_MAJORMINOR/bin/apache-phoenix-$PHOENIX_VERSION-HBase-$HBASE_MAJORMINOR-bin.tar.gz | tar -xz -C /usr/local/
RUN cd /usr/local && ln -s ./phoenix-$PHOENIX_VERSION-HBase-$HBASE_MAJORMINOR-bin phoenix
ENV PHOENIX_HOME /usr/local/apache-phoenix-$PHOENIX_VERSION-HBase-$HBASE_MAJORMINOR-bin
